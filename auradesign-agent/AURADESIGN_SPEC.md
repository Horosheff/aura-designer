...
1. Генерируется целевое высокохудожественное изображение с нужным объектом по промпту.
2. Изображение пропускается через инструмент `recraft_remove_background`, превращаясь в прозрачный PNG.
3. PNG вставляется поверх фоновых градиентов или узоров блока, описанных в CSS. Это позволяет анимировать объект независимо от фона, масштабировать его под любые экраны и избегать неряшливых стыков или жестких границ картинок.
